...
Understand the implementation landscape for both architecture and task breakdown. One question at a time. Follow-ups via `ask_user`.
...
Use `ask_user` with `type: "text"`. Wait for response.
...
Use `ask_user` with `type: "choice"` with 2-4 concrete options based on what you see in the codebase (or code-analysis.md if available).
...
Use `ask_user` with `type: "yesno"` or `type: "text"`. Wait for response. Lock in data models.
...
Use `ask_user` with `type: "yesno"` or `type: "text"`. Wait for response. Lock in contracts.

### 2.4 Design Decisions
For each non-obvious decision, present ONE at a time using `ask_user` with `type: "choice"` and options:
...
Then use `ask_user` with `type: "choice"` and options:
...
Use `ask_user` with `type: "choice"` to confirm:
