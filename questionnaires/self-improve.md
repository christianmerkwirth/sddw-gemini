...
If any root cause origin seems ambiguous, use `ask_user` (type: "choice"):
- "[Origin A] — [reasoning]"
- "[Origin B] — [reasoning]"
...
If "Select which to apply": use `ask_user` (type: "choice", multiSelect: true), listing each IMP-ID as an option.
