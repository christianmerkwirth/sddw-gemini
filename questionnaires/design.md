...
Use `ask_user` with `type: "yesno"` or `type: "text"`. Wait for response. Lock in approved models.
...
Use `ask_user` with `type: "yesno"` or `type: "text"`. Wait for response. Lock in approved contracts.
