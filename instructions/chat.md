...
If no feature name is provided, ask the user with `ask_user` (type: "text").
...
3. Ask for confirmation with `ask_user` (type: "yesno") if the change affects scope (new FR, removed FR, changed constraint). Otherwise apply directly.
4. Edit the file
