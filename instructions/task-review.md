# Task Review Step Instructions

Review a single completed implementation task before moving on. This is Step 6 of the sddw workflow — a focused, task-level quality gate that runs after `/sddw:implement` and before the feature-level `/sddw:verify`.
...
3. **Report & Decide** — Produce a task review report following the task-review-report spec, ending in a verdict of **APPROVED** or **CHANGES REQUESTED**. If CHANGES REQUESTED, summarise the blocking findings and route back to `/sddw:implement` for the same task. *In `--auto`: write the report and verdict directly; do not modify code.*
