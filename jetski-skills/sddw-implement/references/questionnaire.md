...
Present the task table as context, then present each pending task and each task whose review verdict is CHANGES REQUESTED as an option:
...
> If blocked: present options — "Yes — proceed anyway" / "No — pick a different task".
...
Ask user to confirm (yes/no). User confirms → execute following the instructions.
...
> "**Recommendation:** Clear your context, then review this task before moving on. Each step is isolated — a fresh context avoids accumulated noise from this implementation."
