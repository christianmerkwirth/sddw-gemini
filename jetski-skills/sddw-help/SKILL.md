...
name: sddw-help
description: >
  Show sddw workflow overview, list all features with progress indicators,
  or show detailed status for a specific feature. Use when the user says
  "sddw help", "sddw list", "sddw status", "show features", or wants
  to see workflow documentation or feature progress.
---
...
1. **Parse arguments:** Extract `<subcommand>` from the user's message.
2. **Read common rules:** Read `../sddw-common/common-rules.md` for path resolution.

## Routing

Parse the subcommand and route:

- **No argument / "help"** → show workflow overview
...
sddw — Spec-Driven Development Workflow
  ...
  Fast-track:
    sddw design-and-taskify <feature-name>  Combined alias for Design + Taskify
