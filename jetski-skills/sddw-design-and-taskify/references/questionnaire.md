...
Understand the implementation landscape for both architecture and task breakdown. One question at a time.
...
Present 2-4 concrete options based on what you see in the codebase (or code-analysis.md if available).
...
### 2.1 Architecture
Present the architecture overview as context, then offer options:
1. [Proposed architecture — description, data flow]
2. [Alternative approach — description]
...
For each non-obvious decision, present ONE at a time with options:
...
Present the task table as context:
...
2. **Adjust granularity** (break them down more / combine them)
3. **Adjust sequencing** (change dependencies)
...
Present options:
1. **Generate files**
2. **Wait, I need to change something**
