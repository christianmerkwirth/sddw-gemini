...
name: sddw-verify
description: >
  Verify the implementation against requirements, run tests, and create
  remediation tasks if issues are found. Use when the user says "sddw verify",
  "verify feature", "check implementation", or wants to confirm all FRs
  are satisfied after implementation. Requires all prior artifacts to exist.
  Produces .sddw/<feature>/verify/report.md.
...
3. **Report & Remediate** — Produce a verification report following the template at `../sddw-common/specs/verification-report.md`. If any FRs failed or are partial, propose remediation tasks. User confirms which remediation tasks to create. *In `--auto`: create all proposed remediation tasks directly.*
...
Where `<N>` continues the numbering from existing tasks. Follow the same task file format from `../sddw-common/specs/design-task.md`.
