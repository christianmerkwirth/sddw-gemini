...
name: sddw-chat
description: >
  Fast-track interaction with a feature that already has artifacts. Skips the
  full questionnaire ceremony. Use when the user says "sddw chat", or wants
  quick edits, questions, status checks, or small code changes on an existing
  feature without running the full workflow.
  Supports --auto flag to remove even scope-change confirmations.
---
...
If no feature name is provided, ask the user which feature they want to work with.
...
- Architectural deviations (Deviation Rule 4 from implement step)
...
The `--auto` flag is still respected if explicitly passed — it removes even these pauses.

## Process

After loading artifacts, tell the user:
...
3. Ask for confirmation (yes/no) if the change affects scope (new FR, removed FR, changed constraint). Otherwise apply directly.
4. Edit the file
...
2. Follow the TDD Protocol, Commit Protocol, and Deviation Handling from the implement step (read `../sddw-implement/SKILL.md` for the full protocols if needed)
...
> "This looks like it needs a full design pass. Run `sddw design-and-taskify <feature-name>` to break it down properly."

Heuristic: if the change would touch more than 3 files or require more than 1 commit, suggest the full workflow.

## Rules

- SHALL resolve paths using common rules Path Resolution
...
- SHALL follow commit protocol from implement step for any code changes
- SHALL follow deviation handling from implement step for any code changes
...
- SHALL NOT create task files — if the work needs a task file, redirect to `sddw design-and-taskify` or `sddw taskify`
