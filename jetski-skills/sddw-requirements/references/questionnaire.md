...
Present 2-4 interpretations as options when possible. Otherwise ask an open-ended question.
...
Present options:
1. **TDD (Recommended)** — write failing tests first, then implement to pass
2. **Test-after** — implement first, add tests after
3. **Selective TDD** — TDD for business logic/APIs, skip for config/glue code
4. **No tests** — skip testing (e.g., prototype, spike)
...
Ask user to confirm. User confirms → generate spec to `.sddw/<feature-name>/requirements.md`
