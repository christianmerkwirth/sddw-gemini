...
If any root cause origin seems ambiguous, present options:
- "[Origin A] — [reasoning]"
- "[Origin B] — [reasoning]"
...
Present options:
1. "Apply all improvements (Recommended)"
2. "Select which to apply"
3. "Skip — generate report only"

Wait for response.

If "Select which to apply": present each IMP-ID as an option for multi-select.
