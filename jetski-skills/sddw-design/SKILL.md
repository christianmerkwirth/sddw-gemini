...
name: sddw-design
description: >
  Generate the cross-cutting design.md for a feature — architecture, data models,
  interface contracts, and design decisions. Use when the user says "sddw design",
  "design the architecture", or wants to create a technical design document.
  This is Step 3 of the sddw workflow. Requires requirements.md to exist.
  Produces .sddw/<feature>/design/design.md.
  Supports --auto flag for fully autonomous mode.
---

# sddw Design Step

Generate the cross-cutting design artefact — `design.md` — for a feature. This is Step 3 of the sddw workflow. Produces ONLY `design.md`; task files come from the taskify step.
...
- **Interactive mode:** Present options: `Overwrite` / `Edit existing` / `Abort` before writing
...
3. **Confirm & Generate** — Summarise what will be written. User confirms. Generate `design.md` following the spec template at `../sddw-common/specs/design.md`. *In `--auto`: generate directly.*
